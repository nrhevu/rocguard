...
Three components share a single Go module (`module rocguard`, Go 1.25):

1. **Node daemon** (`rocguard daemon`) — runs on every AMD GPU node under
...
2. **CLI** (`cmd/rocguard`) — `run`, `allow`, `status`, `ps`, `register`,
   `token info`, `show-keys`, `bypass`, `revoke`. Single binary shared by
   daemon and CLI; subcommand dispatch is in `cmd/rocguard/main.go`.
...
```
cmd/rocguard/         CLI + daemon entrypoint (single main.go)
...
npm --prefix web/ui run build      # outputs web/ui/dist/, embedded via web/rocguard-ui/
...
- **Module path is `rocguard`** (not `github.com/...`). Internal imports look
  like `rocguard/internal/web`. Keep this style in new code.
- **Config is env-var driven.** All tunables live behind `ROCGUARD_*`
...
- **`KEY=rg_...`** is the user-facing authorization credential;
  **`ROOT_KEY=rk_...`** is the per-node admin secret. Never log either. Audit
  log path is `ROCGUARD_AUDIT_LOG`.
...
- **Secrets never go in source control.** `.dev/`, `*.key`, `web.env`,
  ...
  and the built `rocguard` binary are all gitignored or operator-only.

## Gotchas

- **Never run two enforcing daemons against the same GPUs.** The dev daemon
  must use `--dry-run` and a separate cgroup root
  (`ROCGUARD_CGROUP_ROOT=/sys/fs/cgroup/rocguard-dev`). Production keeps
  `/sys/fs/cgroup/rocguard`.
- **Dev and prod share `/var/lib/rocguard-web` as a bind mount path inside
  the container** but dev points the host side at `.dev/web`. Don't confuse
  the two — dev state must stay under `.dev/`.
- **`rocguard run -- docker run ...` is unsupported.** Docker puts the
  workload in a different cgroup; authorize the container instead with
  `rocguard allow docker --container <name>`. Keep this invariant in any
  enforcement changes.
...
- **The gateway image is `read_only: true` with `cap_drop: [ALL]` and
  ...
  the existing tmpfs `/tmp` or the `/var/lib/rocguard-web` volume — do not
